    ...
    Foundation.libs.topbar = {
        ...
        events: function (bar) {
            ...
            $(this.scope)
              ...
              //    self.toggle(this);
              //})
              .on('click.fndtn.topbar', '[data-topbar] a', function (e) {
                  // toggle menu (collapse it on mobile)
                  var target = $(e.target);
                  if (target[0].nodeName === 'A' && !target.hasClass("no-collapse") && !target.closest("li").hasClass("js-generated")) {
                      var topbar = target.closest('[data-topbar]');
                      if ((target.hasClass('collapse-only') && topbar.hasClass('expanded')) || !target.hasClass('collapse-only')) {
                          self.toggle();
                      }
                  }
              })
